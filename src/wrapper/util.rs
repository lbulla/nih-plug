...
unsafe fn get_flush_zero_mode() -> u32 {
    _mm_getcsr() & std::arch::x86_64::_MM_FLUSH_ZERO_MASK
}

unsafe fn set_flush_zero_mode(x: u32) {
    let val = (_mm_getcsr() & !std::arch::x86_64::_MM_FLUSH_ZERO_MASK) | x;
    _mm_setcsr(val);
}
...
impl ScopedFtz {
    fn enable() -> Self {
        #[cfg(not(miri))]
        {
            ...
            {
                let mode = unsafe { get_flush_zero_mode() };
                let should_disable_again = mode != std::arch::x86_64::_MM_FLUSH_ZERO_ON;
                if should_disable_again {
                    unsafe { set_flush_zero_mode(std::arch::x86_64::_MM_FLUSH_ZERO_ON) };
                }
